...
bool isinteger(const string s) {
  for (unsigned int i = 0; i < s.size(); i++) {
    if (not(isdigit(s[i]) || (i == 0 && (s[i] == '-' || s[i] == '+')
                              && s.size() > 1))) {
      return false;
    }
  }
...
string CalcCaret(ArgListType &arg) {  // CARET_FUN
  if (arg.at(0).first == "-help") {
    Output::Err() << "Calc -help\n\n"
      ...
      "^Calc evaluates all [..],(..),{..} first and then evaluates from\n"
      "left to right.\n"
  ...
  while ((NextNode = CurrentNode->next) != NULL) {
    ...
    unsigned int currSize = CurrentNode->str.size();
    char lastChar = currSize > 0 ? CurrentNode->str[currSize-1] : '\0';
    if (lastChar == 'e' || lastChar == 'E') {
      // we might be dealing with scientific notation
      // It could be 1.07E7, 1.07E+7, 1.07E-7 for example
      string nextstr = NextNode->str;
      if (isinteger(nextstr)) {
        CurrentNode->str = CurrentNode->str + NextNode->str;
        removeNextNode(CurrentNode, NextNode);
      } else if (nextstr == "+" || nextstr == "-") {
        if (NextNode->next && isinteger(NextNode->next->str))  {
          CurrentNode->str = CurrentNode->str + NextNode->str
            + NextNode->next->str;
          removeNextNode(CurrentNode, NextNode);
          removeNextNode(CurrentNode, NextNode);
...
string GetVar(ArgListType &arg) {  // CARET_FUN
  if (arg.size() != 1) {
...
double Calc::Calculate(const CalcNode * StartNode, CalcNode * &EndNode) {
  ...
  if ((StartNode->str == "[") || (StartNode->str == ",")) {
    ...
    while ((StartNode = EndNode) != NULL) {
      ...
      if (StartNode->str == "^") {
        ...
        if ((CurrentArg < 0.0L)
            && (fabs(floor(ArgTwo) - ArgTwo) > verySmallFloat)) {
      ...
      if (StartNode->str == "&") {
        ...
        CurrentArg = (static_cast<bool>(fabs(CurrentArg) > verySmallFloat)
                      && static_cast<bool>(fabs(tempdbl) > verySmallFloat))
          ? 1.0L : 0.0L;
        continue;
      }
  ...
  if (StartNode->str == "~") {
    return fabs(Calculate(StartNode->next, EndNode)) > verySmallFloat
      ? 0.0L : 1.0L;
  }
  ...
  if (isalpha(StartNode->str[0])) {
    ...
    if (StartNode->str == "exists") {
      return (fileExists(StartNode->next->next->str) ||
              (SystemVar::GetVarType(StartNode->next->next->str) != 'u'))
        ? 1.0L : 0.0L;
    }
    ...
    if (StartNode->str == "randint") {
      ...
      return static_cast<double>(getUserRandInt(iround(ArgOne),
                                                iround(ArgTwo)));
    }
    ...
    if (StartNode->str == "mean") {
      ...
      if ((varType == 'M') || (varType == 'A')) {
        DataMatrix Matrix =
          SystemVar::getMatrixOrAnalysis(varName, FunctionName, ComL);
...
double myGamma(double Ex) {
  ...
  const double Ec[] = {-1.910444077728e-03,
                       8.4171387781295e-04,
  ...
  if (Ex <= 0) {
    Enegcheck = 1;
    const double Ey = -1.0 * Ex;
    const double Ey1 = floor(Ey);
    Eres = Ey - Ey1;
