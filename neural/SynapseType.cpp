...
SynapseType::SynapseType(const LearningRuleType learningRule, const float mu,
                         ...
                         const float synFailRate, const string& preNeurType,
                         const string& postNeurType) :
...
SynapseType::SynapseType(const SynapseType& s) :
    m_learningRule(s.m_learningRule), m_mu(s.m_mu), m_NMDArise(s.m_NMDArise), m_alpha(s.m_alpha),
            m_Ksyn(s.m_Ksyn), m_synSuccRate(s.m_synSuccRate), m_preNeurType(s.m_preNeurType),
            m_postNeurType(s.m_postNeurType), m_synapticFilter(s.m_synapticFilter) {
...
void SynapseType::addMember(const string& name, const LearningRuleType learningRule,
                            const float mu, const unsigned int NMDArise, const float alpha,
                            const float Ksyn, const float synFailRate, const string& preType,
                            const string& postType) {
    if (Member.find(name) == Member.end()) {
        Member[name] = SynapseType(learningRule, mu, NMDArise, alpha, Ksyn, synFailRate, preType,
                postType);
    }
    else {
...
void SynapseType::setAlphaDecay() {
    ...
    for (unsigned int i = 0; i < 1001; i++) {
        ...
        for (unsigned int j = 0; j < m_NMDArise + 1; j++) {
            if (m_learningRule == LRT_PostSynB) { //MEGHAN: If using Post Synaptic Rule B set alpha array to ("0" / "the old zBar") until peak, then decay normally
    ...
    if (!m_riseFile.empty()) {
        ...
        if (i != m_NMDArise + 1) { //THIS IS BAD, USER DID NOT INPUT A CORRECT TXT FILE.
            CALL_ERROR << m_riseFile
                    ...
                    << m_riseFile << ",\nor an error in the format of " << m_riseFile << endl
                    << ERR_WHERE;
            exit(EXIT_FAILURE);
        }
    ...
    for (unsigned int i = 0; i < (MAX_TIME_STEP); i++) {
        ...
        if (alphaFallArray[i] < 0.001) {
            m_maxTimeStep = i; // TODO ABH: verify this still does what we want
        }
    }
}
