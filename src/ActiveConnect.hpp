...
void ActiveConnect(ArgListType &arg) {  // AT_FUN
  ...
  static TArg<int> EndPat("-Pend", "end pattern(or vector) {-1 gives end time}",
                          -1);
  static TArg<string> SeqName("-name", "sequence name");
  static TArg<string> ResetSeq("-reset", "reset sequence", "{zeros}");
  static TArg<string> External("-ext", "external sequence", "{zeros}");
  ...
  if (argunset) {
    ComL.HelpSet("@ActiveConnect() \n"
                 "\tGenerates the data about conditional probabilities of\n"
                 "\tfiring and saves the results into the ActiveConBuffer.\n"
                 ...
                 "\tholds the Prob(cij=1 | zi=1,zj=1) for the whole\n"
                 "sequence\n");
  ...
  if (StartPat.getValue() == 1) {
    ...
    for (int inc = 0; inc < StartPat.getValue() - 2; ++inc) {
      ++SeqConstIterator;
    }
    pat_b4 = *(SeqConstIterator++);
  }
  for (int t = StartPat.getValue(); t <= EndPat.getValue(); ++t, ++index,
         ++SeqConstIterator, ++ExtConstIterator) {
  ...
  // Output the data
  Output::Out() << "\nSummed Active Connections in sequence "
                << SeqName.getValue() << " using neurons "
                << StartNeuron.getValue() << "..."
                << EndNeuron.getValue() << " and patterns "
                << StartPat.getValue()
                << "..." << EndPat.getValue()
  ...
  float Pczz = (fabs(TotalSumC) < verySmallFloat) ? 0.0f
    : TotalSumC / TotalSumZ;
