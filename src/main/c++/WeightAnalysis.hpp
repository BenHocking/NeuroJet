...
void WeightAnalysis(ArgListType &arg) {  // AT_FUN
  ...
  static std::string FunctionName = "WeightAnalysis";
  static TArg<int> NumBins("-bins", "number of bins", 15);
  static TArg<std::string> SeqName("-name", "sequence name", "{ones}");
  static FlagArg UsedOnly("-used", "-unused", "include unused neurons", 0);
  static FlagArg Loop("-loop", "-noloop", "assume loop sequence", 0);
  static TArg<int> Con("-con", "calculate only for a certain context"
                       "\n\t\t\t length (-1 ignores)", -1);
  static FlagArg DoSummary("-sum", "-nosum", "print a summary", 1);
  static TArg<double> CutOff("-zero",
  ...
  int NumToAnalyze = 0;
  std::vector<int> NeuronsToAnalyze(ni);
  ...
  double TotalWeights = SystemVar::GetFloatVar("FracConnect")
    * static_cast<double>(ni * ni);
  ...
  float AveAllWeights = static_cast<float>(tmpSum)
    / static_cast<float>(TotalCount);
  ...
  if (Con.getValue() != -1) {
    Output::Out() << " of neurons with "
                  << (Loop.getValue() ? "loop" : EMPTYSTR)
                  << " context length " << Con.getValue() << " from sequence "
                  << SeqName.getValue();
  } else if (UsedOnly.getValue()) {
    Output::Out() << " of neurons that fired from " "sequence "
                  << SeqName.getValue();
  }
