...
void InitializeProgram() {
  ...
  SystemVar::AddStrVar("LearningRuleType", "PostSyn");
  SystemVar::AddStrVar("ThresholdType", "Simple");
  ...
  SystemVar::AddAtFun("ExportVars", ExportVars);
...
bool isNumeric(const string& toCheck) {
  ...
  for (unsigned int i = 0; i < toCheck.length(); ++i) {
    ...
    if (!(isdigit(toCheck[i]) || isDec || isSign || isESym)) {
      toReturn = false;
      break;
    }
  }
...
void CalcSynapticActivation(const UIVectorDeque &FiredArray, const Pattern &inPattern) {
...
#if defined(CHECK_BOUNDS)
      const unsigned int lastC = FanOutCon.at(iFire).at(relTime);
#else
...
void enqueueDendriticResponse(const DataList& dendriticResponse, const DataList& dendResp_inhdiv,
  ...
  for (PopulationCIt it = Population::Member.begin();
    ...
    for (unsigned int i = it->getFirstNeuron(); i < max(it->getLastNeuron() + 1, lastNrn); ++i) {
      dendriteQueue[i].pop_back();
      // AKA push_front()
      dendriteQueue[i].insert(dendriteQueue[i].begin(), dendriticResponse[i]);
      dendriteQueue_inhdiv[i].pop_back();
      dendriteQueue_inhdiv[i].insert(dendriteQueue_inhdiv[i].begin(), dendResp_inhdiv[i]);
      dendriteQueue_inhsub[i].pop_back();
      dendriteQueue_inhsub[i].insert(dendriteQueue_inhsub[i].begin(), dendResp_inhsub[i]);
    }
...
void UpdateParams(const string& varName, const string& varValue,
  ...
  if (varType == 'i') {
    ...
    if (cfValue != varValue) {
      throw invalid_argument(varName + " must be an integer (from -2,147,483,648 to 2,147,483,647)");
    }
...
void resetDendriticQueues() {
  dendriteQueue.assign(ni, DataList(0));
...
void ResetSTM() {
  ...
  Fired.assign(maxAxonalDelay + 1, UIVector(0));
#if defined(MULTIPROC)
  FiredHere.assign(maxAxonalDelay + 1, UIVector(0));
#endif
  ...
  if (SystemVar::GetStrVar("ResetPattern") == EMPTYSTR) {
    ...
    Output::Out() << "Resetting with pattern" << std::endl;
...
void CreateNeuronType(ArgListType& arg) {  // AT_FUN
  ...
  ThresholdType thresholdType = program::parseThresholdType(SystemVar::GetStrVar("ThresholdType"));
...
void CreateSynapseType(ArgListType& arg) {  // AT_FUN
  ...
  LearningRuleType learningRule = program::parseLearningRuleType(SystemVar::GetStrVar("LearningRuleType"));
  ...
  for (unsigned int i = 0; i < Params.size(); i += 2) {
    ...
    if (varName == "NMDAeFoldDecay") alpha = exp(-1/from_string<float>(varV));
    if (varName == "Ksyn") Ksyn = from_string<float>(varV);
    if (varName == "LearningRuleType") learningRule = program::parseLearningRuleType(varV);
  }
...
void CopyData (ArgListType& arg) {  // AT_FUN
  ...
  static int argunset = true;
  static TArg<int> StartPat("-Pstart", "start pattern(or vector)", 1);
  static TArg<int> EndPat("-Pend", "end pattern(or vector) {-1 gives end time}", -1);
  ...
  static TArg<string> DataType("-type", "destination data type {seq,mat,ana,file}", "seq");
  static TArg<string> OldMatName("-from", "data structure to be copied from");
  ...
  static TArg<double> PadVal("-pval", "value to pad with for transpose or for -pad", 0.0f);
...
void ExportVars (ArgListType& arg) {  // AT_FUN
  ...
  static int argunset = true;
  static TArg<string> FileName("-to", "file name");
  static CommandLine ComL(FunctionName);
  if (argunset) {
    ComL.HelpSet("@ExportVars(...) exports variable data to a file.\n");
...
void LoadData (ArgListType& arg) {  // AT_FUN
  ...
  static TArg<string> DataName("-to", "destination data name");
...
void MakeRandSequence(ArgListType& arg) {  // AT_FUN
  ...
  static int argunset = true;
  static TArg<string> SeqName("-name", "new sequence name");
  static TArg<int> SeqLen("-len", "sequence length");
  static TArg<double> ProbFire("-p", "probability of firing");
  static TArg<int> StartNeuron("-Nstart", "start neuron", 1);
  static TArg<int> EndNeuron("-Nend", "end neuron {-1 defaults to ni}", -1);
...
void MakeSequence(ArgListType& arg) {  // AT_FUN
  ...
  static TArg<string> SeqName("-name", "new sequence name");
  static TArg<int> SeqLen("-len", "sequence length");
  static TArg<int> NumOn("-non", "number on in each pattern");
  static TArg<int> Overlap("-ol", "overlap between successive patterns", 0);
  static TArg<int> Stutter("-st", "stutter(does not affect sequence length)", 1);
  static TArg<int> StartNeuron("-Nstart", "start neuron", 1);
  static TArg<int> StartPat("-Pstart", "start pattern", 1);
  static TArg<int> EndPat("-Pend", "end pattern {-1 defaults to -len}", -1);
  static TArg<double> ProbFire("-p", "probability of firing", 1.0);
  static CommandLine ComL(FunctionName);
  if (argunset) {
...
void SaveData (ArgListType& arg) {  // AT_FUN
  ...
  static int argunset = true;
  static TArg<string> DataName("-from", "data structure name");
  static TArg<string> FileName("-to", "file name");
  ...
  if (NodeNum.getValue() == static_cast<int>(ParallelInfo::getRank())) {
    ...
    SystemVar::AddSavedFile(FileName.getValue(), DataName.getValue());
...
void SetLoopVar(ArgListType& arg) {  // AT_FUN
  ...
  static int argunset = true;
  static TArg<string> VarName("-var", "variable name");
  static TArg<string> IterName("-loop", "loop iterator name");
  static TArg<double> StartVal("-start", "starting value");
  static TArg<double> EndVal("-end", "ending value");
  static CommandLine ComL(FunctionName);
  if (argunset) {
    ComL.HelpSet(
...
void Sim(ArgListType& arg) {  // AT_FUN
  ...
  static int argunset = true;
  static TArg<string> FileName("-file", "output file name", "{no file}");
  static TArg<string> xSeqName("-x", "x data name", "TestingBuffer");
  static TArg<string> ySeqName("-y", "y data name", "TrainingBuffer");
  static TArg<string> Method ("-method", "type of similarity {cos,ham,nham}", "cos");
  static TArg<int> StartNeuron("-Nstart", "start neuron", 1);
  static TArg<int> EndNeuron("-Nend", "end neuron {-1 defaults to seq size}", -1);
  ...
  static TArg<int> Winners("-win", "number of winners to show in summary", 3);
...
void Test(ArgListType& arg) {  // AT_FUN
  ...
  static int argunset = true;
  static TArg<string> SeqName("-name", "sequence name", "{zeros}");
  static TArg<int> TimeSteps("-time", "number of time steps to test");
  static TArg<int> StartPat("-Pstart", "start pattern", 1);
  ...
  if (argunset) {
    ...
    ComL.FlagSet(3, &NetType, &InhLearn, &SaveState);
...
void Train(ArgListType& arg) {  // AT_FUN
  ...
  static int argunset = true;
  static TArg<string> SeqName("-name", "sequence name");
  static TArg<int> Trials("-trials", "number of trials to train");
  static TArg<int> SynModBegin("-synModBegin", "timestep to initiate synaptic modification", 1);
...
void Context(ArgListType& arg) {  // AT_FUN
  ...
  IFROOTNODE {
    ...
    static TArg<int> StartNeuron("-Nstart", "start neuron", 1);
    static TArg<int> EndNeuron("-Nend", "end neuron { -1 gives last neuron}", -1);
    static TArg<int> StartTime("-Tstart", "start time", 1);
    static TArg<int> EndTime("-Tend", "end time {-1 gives end time}", -1);
    static TArg<int> Tolerance("-tol", "tolerance", 0);
    static TArg<string> OutFile("-file", "output filename", "{no file}");
    static TArg<string> FiringSequence("-name", "firing sequence name", "TrainingBuffer");
    // DoSummary defaults to -sum(1)
...
string SumData(ArgListType& arg) {  // CARET_FUN
  ...
  static TArg<double> PadVal("-pval", "value to pad with", 0.0f);
  static CommandLine ComL(FunctionName);
  if (argunset) {
...
string PatternLength(ArgListType& arg) {  // CARET_FUN
  ...
  static int argunset = true;
  static TArg<int> Pat("-pat", "pattern(or vector) to find the length of", 1);
  static TArg<string> SeqName("-from", "data structure to use");
  static CommandLine ComL(FunctionName);
  if (argunset) {
